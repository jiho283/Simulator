    ...
    for epi in episodes:
        ...
        for sc_num in session_nums:
            ...
            for un, utter_post in enumerate(post_utterances):
                ...
                save_result, history_num = save_history(history_num, history, history_type, date, cur_conv_num, un, post_utterances, utter_post, model_name, openai_client, model, tokenizer, config, data_dict, ret_method, llama_tokenizer)
                ...
                if random_chatbot_utter.lower() in utter_post.lower() and len(characters) > 0 and target_question_list[0] != 'cannot ask':
                    ...
                    if not save_timeout_flag:
                        ...
                        ret_histories = retrieve_history(ret_method, num_ret_history, openai_client, max_token_len, save_result, char_ask_sh, real_question_sh, data_dict, gt_sessions)
                        retrieve_search_time = time.time()-ret_search_start_time
                        if retrieve_search_time > sleep_time-save_time: # timeout during searching history. Note that saving history was done correctly though.
                        ...
                        # Model inference
                            ...
                            answer = answer_question(model_name, client, model, tokenizer, config, prompt)
