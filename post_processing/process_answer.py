...
def gpt_judge(question, true_answer, answer, client, temperature=0.2, top_p=0.1, patience=3):
    patience = patience
    user_message = open_file("./prompt/gpt_as_a_judge_user_prompt.txt").replace("<<<QUESTION>>>", question).replace("<<<TRUEANSWER>>>", true_answer).replace("<<<PREDICTION>>>", answer)
    result = "Ambiguous"
    while patience > 0:
        completion = client.chat.completions.create(
            ...
            messages=[
            {"role": "system", "content": open_file("./prompt/gpt_as_a_judge_system_prompt.txt")},
...
def calibrate(original_result, is_ambiguous, true_answer_op, answer, question, distilled_answer, answer_format="multi_choice_structured", client=None, lenient=True):
    ...
    Paramters:
        - true_answer_op (str): True answer. one of (A), (B), (C), (D), (E) if multiple choice question. Else it is a text.
